    ...
    with db.transaction(con):
        ...
        if override_applied:
            ...
            db.log_event(
                ...
                object_id=o["id"],
                payload={
                    "note": f"gate override S3->{override_threshold} " f"({operator}): {reason}",
...
def apply_batch(
    ...
    for c in claimed:
        ...
        if obj is not None and not _is_source_fresh(con, obj, root):
            ...
            continue
        author_task = con.execute(
            "SELECT id FROM tasks WHERE object_id=? AND kind='l1_author' "
...
def _load_dep_verdicts(root, run_id, object_id, con) -> dict[str, dict]:
    dt = con.execute(
        "SELECT id FROM tasks WHERE object_id=? AND kind='l1_deepcheck' "
        "ORDER BY id DESC LIMIT 1",
        (object_id,),
    ).fetchone()
    if not dt:
