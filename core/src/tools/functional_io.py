    ...
    for i, c in enumerate(claims):
        ...
        for e in ev:
            ...
            if res.ok:
                ...
                errors.append(
                    f"{cid or i}: evidence {e['path']}:{a}-{b} does not resolve " f"({res.reason})"
                )
        if status == "verified" and not resolved:
            errors.append(
                f"{cid or i}: status 'verified' but no evidence resolves " f"(Check A fail-closed)"
            )
...
def decide_fidelity(
    *, hygiene_ok: bool, fidelity: FidelityResult, other_ns_threshold: int = 2
    ...
    if fidelity.contradictions_high > 0:
        reasons.append(
            f"B/C: {fidelity.contradictions_high} high contradictions "
            f"with L1/process (expected 0)"
        )
    if fidelity.critical_ns_high > 0:
        reasons.append(
            f"PURPOSE/TRIGGER: {fidelity.critical_ns_high} claims "
            f"not_supported high (expected 0)"
        )
    if fidelity.other_ns_high >= other_ns_threshold:
        reasons.append(
            f"claims not_supported high: {fidelity.other_ns_high} "
            f"(threshold {other_ns_threshold})"
        )
    if reasons:
